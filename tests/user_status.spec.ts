...
import { AMM_BASIC_QUERY, MulticallInputs, USER_SHARES_QUERY, multicall, querygql } from "./utils";
const LLAMMA_ABI = require("../abis/LLAMMA.json");
const Controller_ABI = require("../abis/Controller.json");

let user_tick_numbers_total: any[] = [];
let user_count = 0;
let active_band = 0;
let min_band = 0;
let max_band = 0;
...
describe("User Status", function() {
  ...
  it("should every user's n1, n2, ticks correct", async function() {
    ...
    user_count = Number(base_res.data.amms[0]["user_count"]);
    active_band = Number(base_res.data.amms[0]["active_band"]);
    min_band = Number(base_res.data.amms[0]["min_band"]);
    max_band = Number(base_res.data.amms[0]["max_band"]);
    ...
    for (let loopIndex = 0; loopIndex < batch_num; loopIndex++) {
      ...
      const user_states = await multicall(inputs_user_state, Controller_ABI);

      user_tick_numbers_total = user_tick_numbers.concat(
        user_tick_numbers
      ) as any[];
      ...
      for (let i = 0; i < user_tick_numbers.length; i++) {
        ...
        if (cur_key in gql_user_shares) {
          ...
          for (let band_index = 0; band_index <= N.toNumber(); band_index++) {
            expect(
              gql_ticks[i] === gql_n1 + i,
              `should ticks[${i}].index equals band index`
            );
          }
        }
      }
